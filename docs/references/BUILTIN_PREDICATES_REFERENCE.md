...
`format("~w", X)` treats a non-list second argument as one argument. (4.5.0 removed three of the
4.3 deviations: `call((fail, 1))` raises `type_error(callable, (fail,1))` (ISS-2025-0518),
`string_concat(X, Y, Z)` with nothing bound raises `instantiation_error` (ISS-2025-0596), and
`tab/1,2` evaluates its count (ISS-2025-0604).) The complete list of deliberate deviations is
`docs/references/ref-deviations.md`.

### 62.1 The extended libraries (4.6 wave Q1)

Wave Q1 (ISS-2025-0680..0699) closed LIM-038: the bridged extended libraries — jdbc, filesystem,
network, http, crypto, csv, datetime, json, os, persistence, regex, xml, graph, the DCG extension
predicates (`phrase_with_options/4`, `enhanced_phrase/2,3`), the debug predicates (`leash/1` & co.)
and the bridged half of `io` — raise ISO `error/2` terms for every argument fault. Their context
is SWI-Prolog's, `context(Name/Arity, Message)`: the English sentence the library used to throw as
a bare atom is kept as the context's message.

| Goal | before 4.6 | 4.6 |
|---|---|---|
| `xml_parse(1, X)` | `'xml_parse: argument must be an atom.'` | `error(type_error(atom, 1), context(xml_parse/2, _))` |
| `xml_parse('<a', X)` | the parser's message atom | `error(syntax_error(xml), context(xml_parse/2, Msg))` |
| `re_match('(', abc)` | the ATOM `'error(syntax_error(invalid_regex), re_match/2): ...'` | `error(syntax_error(invalid_regex), context(re_match/2, _))` |
| `md5_hash(f(x), H)` | `'md5_hash: Text must be an atom.'` | `error(type_error(atom, f(x)), context(md5_hash/2, _))` |
| `crypto_hash(sha999, abc, H)` | message atom | `error(domain_error(hash_algorithm, sha999), _)` |
| `date_add(D, 1, fortnights, R)` | message atom | `error(domain_error(date_unit, fortnights), _)` |
| `json_parse('{"a":', J)` | message atom | `error(syntax_error(json), context(json_parse/2, _))` |
| `json_keys(foo, K)` | message atom | `error(type_error(json, foo), _)` |
| `sleep(a)` | message atom | `error(type_error(number, a), context(sleep/1, _))` |
| `jdbc_connect(X, C)`, `tcp_connect(X, 80, S)`, `file_exists(X)`, ... | message atoms | `instantiation_error` |
| `persist(foo)` | message atom | `error(type_error(predicate_indicator, foo), _)` |
| `leash(bogus)` | message atom | `error(domain_error(leash_mode, bogus), _)` |
| `graph_vertices([f(x)], V)` | message atom | `error(type_error(edge, f(x)), graph)` |

**A host failure is not an argument fault.** When the file system, a socket, a database or a
process fails, the library raises the SWI formal for it (`core.engine.v4.Errors.host`):

| Host failure | Error term |
|---|---|
| a file or directory that does not exist | `existence_error(file, Path)` / `existence_error(directory, Path)` |
| access denied / a security manager refusal | `permission_error(Action, file, Path)` |
| creating a directory that exists / deleting a non-empty one | `permission_error(create, directory, P)` / `permission_error(delete, directory, P)` |
| an unknown host | `existence_error(host, Host)` |
| a missing JDBC driver or FFI class | `existence_error(class, Name)` |
| an unknown handle (server, request, socket, snapshot, JDBC connection/statement) | `existence_error(Kind, Handle)` |
| any other I/O failure (connection refused, broken pipe, ...) | `io_error(Operation, Culprit)` |
| a SQL error, a crypto provider error, anything else | `system_error(Message)` |

**Exact arities (Q1.2, ISS-2025-0685).** Every registered name now declares the arities it
implements, so a call at any other arity is an unknown procedure — `char_code(X)` raises
`existence_error(procedure, char_code/1)` instead of `'char_code/2 requires exactly 2 arguments'`,
and a program may define its own `char_code/1`. `assertz/1` on every REAL arity of a built-in is
still `permission_error(modify, static_procedure, PI)`.

**Thread join status (Q1.3, ISS-2025-0698).** A worker stopped by the inference budget reports
`exception(error(resource_error(inference_limit), inference_limit_exceeded))` in its
`thread_join/2` status (it was the bare atom `exception(inference_limit_exceeded)`). Inside the
exhausted query the budget stays uncatchable (the trust model); the join status is read by
another query, so a term is safe there.

`ExtendedLibraryErrorsTest` probes every registered name at arities 0..4 with every argument
unbound and with a wrong-type first argument (3 825 goals) and fails on any message-atom error or
arity-guard message: before wave Q1 it counted 368 message atoms and 1 089 arity-guard messages,
now 0 and 0.
...
- **Constraint solving** with CLP(FD) finite domain constraints (v3.0.0 interval-domain solver; v4.5.0: domains with holes, `inf..sup`, reification, global constraints, lazy SWI-style labeling with branch and bound), bound-consistency propagation
