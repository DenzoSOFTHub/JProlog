...
public class TermBuiltinsTest {
    ...
    public void testCopyTerm() {
        ...
        // Test copy_term with a simpler case
        List<Map<String, Term>> solutions = prolog.solve("copy_term(f(a, b), Copy).");
        assertEquals(1, solutions.size());
        assertNotNull(solutions.get(0).get("Copy"));
        
        // Test that the copy was made
        Term copy = solutions.get(0).get("Copy");
        assertTrue(copy.toString().contains("f"));
    }
