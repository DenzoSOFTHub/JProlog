...
import it.denzosoft.jprolog.core.utils.ListTerm;
...
public class PrologParser {
    ...
    private Term parsePrimary() throws PrologParserException {
        ...
        if (token.matches("[a-z].*")) {
            ...
            Term term = parseTerm();
            if (!peek().equals(")")) {
                throw new PrologParserException("Expected ')'");
            }
            nextToken(); // Consume ')'
            return term;
        } else {
            throw new PrologParserException("Unexpected token: " + token);
        }
    }

    private Term parseAtomOrCompoundTerm() throws PrologParserException {
        ...
        if (peek() != null && peek().equals("(")) {
            ...
            while (true) {
                ...
                String token = peek();
                if (token == null) {
                    throw new PrologParserException("Unclosed parenthesis in compound term");
                } else if (token.equals(")")) {
                    nextToken(); // Consume ')'
                    break;
                } else if (token.equals(",")) {
                    ...
                    throw new PrologParserException("Unexpected token in compound term: " + token);
                }
            }
    ...
    private Number parseNumber() throws PrologParserException {
        ...
        try {
            ...
            throw new PrologParserException("Invalid number format: " + numberString);
        }
    }
    ...
    private ListTerm parseList() throws PrologParserException {
        ...
        while (true) {
            String token = peek();
            if (token == null) {
                throw new PrologParserException("Unclosed list");
            }
            if (token.equals("]")) {
                nextToken(); // Consume ']'
                break;
            ...
            if (element != null) {
                ...
                throw new PrologParserException("Expected term in list");
            }
    ...
    public static class PrologParserException extends Exception {
