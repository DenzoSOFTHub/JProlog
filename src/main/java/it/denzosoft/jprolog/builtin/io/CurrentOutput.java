...
/**
 * current_output/1 - current_output(?Stream), ISO 8.11.2.
 * Unifies with {@code user_output} for the standard stream and with {@code '$stream'(N)} otherwise.
 */
public class CurrentOutput implements BuiltIn {
    ...
    public boolean execute(Term query, Map<String, Term> bindings, List<Map<String, Term>> solutions) {
        if (query.getArguments().size() != 1) {
            throw new PrologEvaluationException("current_output/1 requires exactly 1 argument");
        }
        Streams st = StreamManager.streams();
        PrologStream s = st.currentOutput();
        Term value = (s == st.userOutput()) ? new Atom("user_output") : Streams.termFor(s);
