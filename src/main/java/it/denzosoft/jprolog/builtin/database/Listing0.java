...
public class Listing0 implements BuiltInWithContext {
    ...
    public boolean executeWithContext(QuerySolver solver, Term query, Map<String, Term> bindings, List<Map<String, Term>> solutions) {
        ...
        if (query.getArguments().size() != 0) {
            throw new it.denzosoft.jprolog.PrologEvaluationException("listing/0 takes no arguments.");
        }
        ...
        if (engine != null) {
            engine.listing();
             ...
             throw new it.denzosoft.jprolog.PrologEvaluationException("listing/0: Prolog engine context not available.");
        }
